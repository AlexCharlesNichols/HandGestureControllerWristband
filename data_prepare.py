...
folders = ["Arm_Raise", "Rotation_CW","Tapping"]
names = [
    "Dean", "Edin", "Lachlan", "Amila", "Lynette", "Sabina"
]
  ...
  if folder != "negative":
    with open(file_to_read, "r") as f:
      ...
      for idx, line in enumerate(lines):  # pylint: disable=unused-variable,redefined-outer-name
        if len(line) == 4:
          if line[0][0] == "T" and data_new[DATA_NAME]:
            data.append(data_new)
            data_new = {}
            data_new[LABEL_NAME] = folder
            data_new[DATA_NAME] = []
            data_new["name"] = name
          elif line[0][0] != "T" and line[0][0] != " ":
            data_new[DATA_NAME].append([float(i) for i in line[1:4]])
      data.append(data_new)
  else:
    with open(file_to_read, "r") as f:
      lines = csv.reader(f)
      ...
      for idx, line in enumerate(lines):
        if len(line) == 4 and line[0][0] != "T" and line[0][0] != " ":
  ...
  for idx1, folder in enumerate(folders):
    for idx2, name in enumerate(names):
      prepare_original_data(folder, name, data,
                            "./data/%s/%s_%s.csv" % (folder,name, folder))
  for idx in range(6):
    prepare_original_data("negative", "negative%d" % (idx + 1), data,
                          "./data/negative/Lachlan_Negative_%d.csv" % (idx+1))
