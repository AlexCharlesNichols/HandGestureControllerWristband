...
def reshape_function(data, label):
  reshaped_data = tf.reshape(data, [-1, 3, 1]) #changed from [-1, 3, 1]
...
def build_cnn(seq_length):
  ...
  model = tf.keras.Sequential([ 
      ...
      tf.keras.layers.Conv2D(16, (4, 1), padding="same", ## VALUE WAS ORIGINALLY 4 BUT CHAMGED TO 5 
      ...
      tf.keras.layers.Dense(4, activation="softmax")  # (batch, 4) ## VALUE WAS ORIGINALLY 4 BUT CHAMGED TO 5 
  ])
...
def train_net(
  ...
  confusion = tf.math.confusion_matrix(
      ...
      num_classes=4) ##changed from 4 to 5 
  print(confusion)
  ...
  #tflite_model = converter.convert()
  #open("quantizationmodel.tflite", "wb").write(tflite_model)
  # Save the model to disk
  ...
  if kind =="CNN":
    model.save("./savedmodel/CNNmodel.h5") #saving full tensorflow model

    converter = tf.lite.TFLiteConverter.from_keras_model(model) #converting to TFlite
    converter.target_spec.supported_ops = [
      tf.lite.OpsSet.TFLITE_BUILTINS, # enable TensorFlow Lite ops.
      tf.lite.OpsSet.SELECT_TF_OPS # enable TensorFlow ops.
    ]
    tflite_model = converter.convert()
    open("./savedmodel/converted_CNNmodel.tflite", "wb").write(tflite_model)  
    
  if kind =="LSTM":
    model.save("./savedmodel/LTSMmodel.h5") #saving full tensorflow model
    ...
    tflite_model = converter.convert()
    open("./savedmodel/converted_LSTMmodel.tflite", "wb").write(tflite_model)

  #open("model_quantized", "wb").write(model) ##.tflite 
...
if __name__ == "__main__":
  ...
  seq_length = 128 ## was 128 changed by alex to 354
